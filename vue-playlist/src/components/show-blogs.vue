...
<template>
    <div id="show-blogs">
        ...
        <!-- Blog başlıklarında arama yapmak için input -->
        <input type="text" v-model="keyword" placeholder="Bloglarda ara...">
        
        <!-- Filtrelenmiş blogları göster -->
        <div v-for="blog in filteredBlogs" :key="blog.id" class="single-blog">
            <!-- Blog başlıklarını büyük harfe çevirir, rastgele renk verir ve ilgili blog detayına yönlendirir -->
...
export default {
    ...
    computed: {
        ...
        filteredBlogs() {
            return this.blogs.filter(blog => blog.title.toLowerCase().includes(this.keyword.toLowerCase()));
        }
    },
    created() {
